...
namespace Fusee.Tutorial.Core
{
    public class HierarchyInput : RenderCanvas
    {
        ...
        private TransformComponent _flinksTransform;
        private TransformComponent _frechtsTransform;
    ...
    SceneContainer CreateScene()
    {
        ...
        _flinksTransform = new TransformComponent
        {
            Rotation = new float3(0.5f, 0, 0),
            Scale = new float3(0.5f, 0.5f, 1),
            Translation = new float3(-1.5f, 8, 0)
        };
        _frechtsTransform = new TransformComponent
        {
            Rotation = new float3(0.5f, 0, 0),
            Scale = new float3(0.5f, 0.5f, 1),
            Translation = new float3(1.5f, 8, 0)
        ...
        public override void RenderAFrame()
        {
            ...
            float bodyRot = _bodyTransform.Rotation.y;
                ...
                foreArmRot += 0.1f * Keyboard.WSAxis;
                _foreArmTransform.Rotation = new float3(foreArmRot, 0, 0);

                float flinks = _flinksTransform.Rotation.x;
                flinks += Keyboard.ADAxis * DeltaTime * 14;
                if(flinks > -0.6){
                    flinks += Keyboard.ADAxis * DeltaTime * -14;
                }
                if(flinks < 1){
                ...
                _flinksTransform.Rotation = new float3(0, 0, flinks);

                float frechts = _frechtsTransform.Rotation.x;
                frechts += Keyboard.ADAxis * DeltaTime * 14;
                if(frechts > 1){
                    frechts += Keyboard.ADAxis * DeltaTime * -14;
                }
                if(frechts < -0.6){
                ...
                _frechtsTransform.Rotation = new float3(0, 0, frechts);
